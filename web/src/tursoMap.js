/** Turso snake_case → SPA keys.
 * Keep original keys (VSS/MSC use snake_case) AND add camelCase aliases (DAV).
 */
...
/** Return value if it looks like a sane calendar date; else null. */
...
const VSS_DATE_KEYS = ['tungay_hd', 'denngay_hd', 'congbo', 'tungay', 'denngay', 'created_date',
...
export function mapTursoRow(row) {
  ...
  if (out.tagId != null && Array.isArray(out.tagId)) out.tagId = out.tagId[0] || ''
  if (out.tag_id != null && Array.isArray(out.tag_id)) out.tag_id = out.tag_id[0] || ''
